...
在[!UICONTROL 客户配置]仪表板中，您可以自定义类别（如业务部门或产品线）、跟踪其他品牌和添加品牌提及别名，以跨提示捕获品牌的所有变体。 这一设置确保平台根据您的业务环境定制洞察，从而实现准确的可视性、流量和机会分析。

## 行业提示库

为了帮助您开始使用提示和主题，Adobe创建了一个行业提示库，该库通过与行业专家进行广泛的研究以及对6,000多个客户的AI 搜索行为分析来开发。 该库基于行业特定趋势、已验证的业务目标以及真实世界的客户搜索模式，识别最相关的主题和提示。
...
* **客户情报：**&#x200B;对6,000多个客户的AI 搜索行为和偏好设置分析
