...
代理流量仪表板显示AI代理(爬虫和聊天机器人)如何与您的网站进行交互。 通过使用此视图，您可以跟踪请求总数以及与性能相关的一般指标。 您还可以查看跨市场、类别、页面和代理的流量分布。 此仪表板使用的数据源自CDN日志，因此您必须配置&#x200B;**CDN日志转发**&#x200B;才能显示指标。 还有一些可自定义的过滤器，可帮助您优化显示的数据。
...
这些规范化字段通过代理视图公开。 在[引荐流量](/help/dashboards/referral-traffic.md)仪表板上，CDN日志用于显示页面点击量度。 在CDN日志摄取或后续数据处理的任何阶段中，不会处理或存储任何个人身份信息(PII)。
...
* **平台** — 选择要分析的AI引擎。
* **代理类型** — 按与您的网站交互的AI代理类型进行筛选。 您可以在爬虫、聊天机器人或所有代理之间筛选。
...
用户代理和URL性能分析视图提供了有关爬虫和聊天机器人如何与您的网站交互的更多数据划分。 单击以下选项卡以了解详细说明。
...
“用户代理分析”表提供按页面类型和代理类型(例如，爬虫与聊天机器人)划分的流量。 这样一来，便可以轻松了解哪些AI代理抓取了网站的哪些部分。 它包含以下类别：

* **页面类型** — 页面类型。
* **代理类型** - 抓取页面的AI代理，爬虫或聊天机器人。
...
“URL性能分析”表显示各个URL的详细视图。 这包括点击量、唯一代理、顶级代理、成功率和类别。 这样，您就可以识别高价值页面、检测抓取差距并为AI引擎优化内容。 URL按流量排名。 该表包含以下类别：
...
URL详细信息视图提供对页面性能的整体了解，显示页面的引用频率、AI响应中引用该页面的情绪、它出现的主题和提示以及代理和引荐流量随时间变化的趋势。

>[!ENDTABS]

在这两个表中，您可以使用&#x200B;**导出**&#x200B;选项下载表`.csv`并与团队共享见解，或者在执行报表中包含表。
