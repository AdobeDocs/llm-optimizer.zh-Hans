...
* **具有自有品牌引用的独特提示数** — 具有自有品牌引用的独特AI提示总数。
* **唯一提示总数** — 唯一AI提示总数。
...
对于被引用视图和趋势视图，URL在每行的末尾都有一个&#x200B;**详细信息**&#x200B;按钮。 单击该按钮将显示一个单独的窗口，其中包含其他详细信息。 详细信息窗口显示URL被引用的频率、<!--the sentiment of AI responses where it is mentioned,-->其中出现的主题和提示，以及代理和引荐流量随时间变化的趋势（适用于拥有的URL）。
