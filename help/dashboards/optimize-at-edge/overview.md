...
如果您在生产之前对暂存主机名进行测试，请使用 LLM Optimizer 注册&#x200B;**一个**&#x200B;暂存域并从&#x200B;**客户配置** → **内容传递网络配置** → **将优化部署到 AI 代理** → **添加暂存域**（或&#x200B;**暂存域**）复制其&#x200B;**暂存** Edge Optimize API 密钥。暂存主机名必须与生产网站共享同一可注册域。**自带内容传递网络**&#x200B;指南包括检索暂存密钥和验证暂存 URL 上的路由的完整步骤。
